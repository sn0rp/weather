...
export async function getWeatherData(lat: number, lon: number): Promise<WeatherData> {
  ...
  const hourlyData = weather.hourly.time
    ...
    .map((time: string, i: number) => ({
      ...
      wind: Math.round(weather.hourly.windspeed_10m[i + currentIndex]),
      ...
      visibility: Math.round(weather.hourly.visibility[i + currentIndex] / 1000),
  ...
  const dailyData = weather.daily.time.map((time: string, i: number) => {
    ...
    return {
      ...
      wind: Math.round(weather.hourly.windspeed_10m[dayStart]),
      ...
      visibility: Math.round(weather.hourly.visibility[dayStart] / 1000),
